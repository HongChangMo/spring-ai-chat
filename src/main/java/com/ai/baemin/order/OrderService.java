...
public class OrderService {
    ...
    public String requestRefund(String orderId, String reason) {
        return orderRepository.findById(orderId)
                .map(o -> {
                    if (!o.status().isRefundable()) {
                        return String.format("주문 %s은(는) %s 상태로 환불할 수 없습니다.", orderId, o.status().label());
                    }
                    return String.format("주문 %s에 대해 '%s' 사유로 환불을 요청하시겠습니까? 확인하시면 환불을 진행합니다.", orderId, reason);
                })
                .orElse(notFound(orderId));
    }
    ...
    private String notFound(String orderId) {
